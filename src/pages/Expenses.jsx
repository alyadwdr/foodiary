...
export default function Expenses() {
  ...
  const [form, setForm] = useState({ name: '', amount: 1, price: '', notes: '' })
  ...
  function openAdd() {
    setForm({ name: '', amount: 1, price: '', notes: '' })
    setModal('add')
  }
  function openEdit(exp) {
    setForm({ name: exp.name, amount: exp.amount, price: exp.price, notes: exp.notes || '' })
    setModal(exp)
  }

  async function handleSave() {
    const payload = { name: form.name, amount: Number(form.amount), price: Number(form.price), notes: form.notes }
  ...
  function handleExportPDF() {
    exportToPDF(
      ...
      filtered.map((e, i) => ({ no: i + 1, date: formatDate(e.created_at), name: e.name, amount: e.amount, price: formatCurrency(e.price), notes: e.notes || '-' })),
  ...
  function handleExportExcel() {
    exportToExcel(
      ...
      filtered.map((e, i) => ({ no: i + 1, date: formatDate(e.created_at), name: e.name, amount: e.amount, price: e.price, notes: e.notes || '' })),
      `expenses_${filter.toLowerCase().replace(/ /g, '_')}`
    )
  }
    ...
    <div className="p-4 lg:p-8 space-y-6 max-w-7xl mx-auto">
                ...
                <tr key={exp.id} className="table-row hover:bg-cream-lighter/50 transition-colors">
                  ...
                  <td className="table-cell">
                    <div className="flex gap-1.5">
                      ...
                      <button onClick={() => handleDelete(exp.id)} className="w-7 h-7 flex items-center justify-center rounded-xl hover:bg-red-50 transition-colors">
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === 'add' ? 'New Expense' : 'Edit Expense'}>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-cocoa/60 mb-1.5 block">Item Name</label>
